...
!> The cpp_aerosol_t type and related functions
module ai_cpp_aerosol
  ...
  public :: cpp_aerosol_t

  type, extends(aerosol_t) :: cpp_aerosol_t
    type(c_ptr) :: model_
  contains
    procedure :: get_optics
  end type cpp_aerosol_t

  interface cpp_aerosol_t
  ...
  interface
    ...
    function ai_bridge_cpp_supports_aerosol( package_name )   &
        result( exists ) bind (c)
      use iso_c_binding
      logical(kind=c_bool)                 :: exists
      character(kind=c_char), dimension(*) :: package_name
    end function ai_bridge_cpp_supports_aerosol

    !> Builds a C++ aerosol model
    function ai_bridge_cpp_new_aerosol( package_name, description_file )   &
        result( aerosol ) bind (c)
      use iso_c_binding
      type(c_ptr)                          :: aerosol
      character(kind=c_char), dimension(*) :: package_name
      character(kind=c_char), dimension(*) :: description_file
    end function ai_bridge_cpp_new_aerosol
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  function constructor( package_name, description_file ) result( aerosol )
    ...
    type(cpp_aerosol_t),  pointer    :: aerosol
    character(len=*),     intent(in) :: package_name
    character(len=*),     intent(in) :: description_file

    if( ai_bridge_cpp_supports_aerosol( trim( package_name )//c_null_char ) ) &
      then
      allocate( aerosol )
      aerosol%model_ = ai_bridge_cpp_new_aerosol(                             &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine get_optics( this, optics )
    ...
    !> Aerosol model
    class(cpp_aerosol_t),   intent(inout) :: this
