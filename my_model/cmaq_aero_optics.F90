...
MODULE cmaq_aero_optics


     Integer      :: AERO_UTIL_LOG = 0 
private
public :: aero_optical, aero_optical_mie, aero_optical_core_shell
...
!     set of refractive indices.
      ...
      IMPLICIT NONE
! *** input variables
      real(8), intent(in)    :: lamda_in               ! wavelengths  [micro-m]
...
! *** initialize variables
       lamdam1 = 1.0e6 / real(lamda_in)   ! lamda now in [ m ]
       bext    = 0.0
       bscat   = 0.0
       sum_g   = 0.0
...
! ------------------------------------------------------------------
       subroutine aero_optical_mie( lamda_in, crefin, Vol, dgn, &
...
! *** input variables
      ...
      real(8), intent(in)    :: Vol        ! modal aerosol volumes [m**3 /m**3]
      real(8), intent(in)    :: dgn        ! geometric mean diameters 
                                        ! for number distribution [ m]
      real(8), intent(in)    :: sig        ! geometric standard deviation 
      
! *** output variables 
      real(8), intent(out)    :: bext         ! extinction coefficient [ 1 / m ]
      real(8), intent(out)    :: bscat        ! scattering coefficient [ 1 / m ]
      real(8), intent(out)    :: gfac         ! assymetry factor for Mie and molecular scattering
      logical, intent(out) :: success      ! flag for successful calculation
...
!     calculate Mie size parameter for volume distribution
!     exp(3.0 * xlnsig*xlnsig)  converts dgn to dgv (volume diameter)
       alphav =  pi * real(dgn) * exp(3.0 * LSIGX * LSIGX) * real(lamdam1)
...
!     by multplying by the modal volume and dividing by the wavelength
       ...
       vfac  =  Vol * lamdam1         
       bext    = vfac * real(beta_Ex,8)  ! [ 1 / m ]
       bscat   = vfac * real(beta_Sc,8)  ! [ 1 / m ]
       gfac    = real(G,8)
...
99504  Format('Even Number Quadrature Points for Volume Averaged Aerosol Optics')
       ...
       END SUBROUTINE aero_optical_mie

! ------------------------------------------------------------------
       subroutine aero_optical_core_shell ( lamda_in, refcor,refshell, VOLCOR,   &
...
! *** input variables
       ...
       real(8),intent(in) ::  VOLCOR    ! volume of core
       real(8),intent(in) ::  VOLSHELL  ! volume of shell
       real(8),intent(in) ::  DGNCOR    ! geometric mean diameters  
                                        ! for number distribution [m]
       real(8),intent(in) ::  DGNSHELL  ! geometric mean diameters  
                                        ! for number distribution [m]
       real(8),intent(in) ::  SIG       ! geometric standard deviation 
...
! *** output variables 
       real(8),intent(out)  ::  bext      ! extinction coefficient [ 1 / m ]
       real(8),intent(out)  ::  bscat     ! scattering coefficient [ 1 / m ]
       real(8),intent(out)  ::  gfac      ! assymetry factor 
       logical, intent(OUT) :: success    ! flag for successful calculation
...
!    calculate the extinction and scattering coefficients
       LSIGX  = log(real(SIG))
       expfac = pi * exp(3.0 * LSIGX * LSIGX) * real(lamdam1)
        
!     calculate Mie size parameter for volume distribution
!     exp(3.0 * xlnsig*xlnsig)  converts dgn to dgv (volume diameter)
       XX =  real(DGNCOR) * expfac                                       
       YY =  real(DGNSHELL) * expfac
       ...
       If( Use_Odd_Quadrature )then
           ...
           print *,'refcor=',refcor,' refshell=',refshell
           print *,'XX=',XX,'  YY=',YY
           print *,'LSIGX=',lsigx
...
!     volume         
       print *,'beta_Ex=',beta_Ex,'  beta_Sc=',beta_Sc,'  G=',G
...
99502  Format('Even Number Quadrature Points for Core-Shell Aerosol Optics')
       END SUBROUTINE aero_optical_core_shell
